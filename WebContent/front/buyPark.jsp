...
<body>
							...
							<form class="form-horizontal" style="margin-left: 100px;">
								...
								<div class="form-group">
									<div class="col-sm-offset-2 col-sm-2">
										<button type="button" style="width: 100px;" id="buyPark"
	...
	<script>
	$("#parkingId").bind("change",function(){
		  ...
		  flow.load({
		    ...
		    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
		      ...
		      $.ajax({
     			 url:"${pageContext.request.contextPath}/FrontPackingServlet?op=queryParkById",
     			 type:"get",
     			 data:{
		        ...
		        //假设你的列表返回在data集合中
		          ...
		          layui.each(array, function(index, park){
		        	  ...
		        	  text += "											<a";
		        	  text += "												 node-type=\"sima-click-0\">车位所在小区:"+park.villageName+"</a>";
		        	  text += "										</h3>";
		        	  text += "										<p class=\"ty-card-tip1\">车位号："+park.parkingNo+"    车位面积:"+park.area+"</p>";
		        	  text += "										<p class=\"ty-card-tip2 clearfix\">";
		        	  text += "											<span class=\"ty-card-tip2-i ty-card-time\">费用：500</span><span";
	       ...
	       $("#buyPark")
			.click(
					function() {
						console.log(111);
						$.ajax({
									url : "${pageContext.request.contextPath}/FrontPackingServlet?op=updatePark",//url地址
									type : "post",
									data : {
										"parkingId" : $("#parkingId").find("option:selected").val(),
...
$(function() {
	//ajax的 get请求
	$.get("${pageContext.request.contextPath}/FrontPackingServlet?op=queryParkByStatu", function(data, status) {
		//使用js的内置对象JSON将返回的值转化为数组
		array = JSON.parse(data);
		//遍历数组
		$.each(array, function(index, park) {
			$("#parkingId").append(
					"<option value="+park.parkingId+">" + park.parkingNo
							+ "</option>");
		});
	});
});
