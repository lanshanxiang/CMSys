					...
					<div class="content w1150" id="content"
						...
						<h1 style="color: dodgerblue;">账单查缴</h1>
							...
							<ul class="nav nav-tabs">
								<li class="active">
									<a data-toggle="tab" href="#electric">电费查询</a>
								</li>
								<li>
									<a data-toggle="tab" href="#water">水费查询</a>
								</li>
								<li>
									<a data-toggle="tab" href="#other">查询其他费用</a>
								</li>
						</ul>
								...
								<div class="form-group">
									<div class="col-sm-offset-2 col-sm-2">
										<button style="width: 100px;" id="queryMyElectric"
											type="button" class="btn btn-success">提交</button>
									</div>
								</div>
							</form>
							<div class="content w1150" id="content"
		    ...
		    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
		      ...
		      $.ajax({
     			 ...
     			 data:{
     				...
     				"years" : $('#years').val(),
     				"months" : $('#months').val(),
     				"tenementId" : ${sessionScope.user.userId},
     				 "page":page
     			 },
     			 dataType:"text",
		    ...
		    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
		      ...
		      $.ajax({
     			 ...
     			 data:{
     				...
     				"years" : $('#years').val(),
     				"months" : $('#months').val(),
     				"tenementId" : ${sessionScope.user.tenementId},
     				 "page":page
     			 },
     			 dataType:"text",
     			 success:function(res){
		...
		.click(
		  ...
		  flow.load({
		    ...
		    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
		      ...
		      //以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
		      $.ajax({
     			 url:"${pageContext.request.contextPath}/FrontPaymentServlet?op=myOther",
     			 type:"get",
     			 data:{
     				///"tenementId" : $('#tenementId').val(),
     				"years" : $('#years').val(),
		        ...
		        //假设你的列表返回在data集合中
		          ...
		          layui.each(array, function(index, payment){
		        	  ...
		        	  text += "										<h3 class=\"ty-card-tt\">";
		        	  text += "											<a";
		        	  text += "												 node-type=\"sima-click-0\">住户:"+payment.tenementName+"    费用类型："+payment.costName+" </a>";
		        	  text += "										</h3>";
		        	  text += "										<p class=\"ty-card-tip1\">年份："+payment.years+"    月份:"+payment.months+"</p>";
		        	  text += "										<p class=\"ty-card-tip2 clearfix\">";
		        	  text += "											<span class=\"ty-card-tip2-i ty-card-time\">费用："+payment.payable+"</span><span";
