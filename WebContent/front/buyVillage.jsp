...
<body>
						...
						<div class="content__article">
							...
							<form class="form-horizontal" style="margin-left: 100px;">
								<div class="form-group">
									<label for="villageId" class="col-sm-2 control-label">请选择要您喜欢的小区</label>
									<div class="col-sm-4">
										<select class="form-control" name="villageId" id="villageId">
                                             
										</select>
									</div>
									<button id="btnQueryVillage" class="btn btn-success">查询</button>
								</div>
								<div class="content__list" id="queryVillage"></div>
								<div class="form-group">
									<div class="col-sm-offset-2 col-sm-2">
										<button style="width: 100px;" id="buyVillage" type="button"
	...
	<script>
	 $("#btnQueryVillage").click(function() { layui.use('flow', function(){
		  var $ = layui.jquery; //不用额外加载jQuery，flow模块本身是有依赖jQuery的，直接用即可。
		  var flow = layui.flow;
		  flow.load({
		    elem: '#queryVillage' //指定列表容器
		    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
		      ...
		      $.ajax({
     			 ...
     			 type:"get",
     			 data:{
     				"villageId" : $("#villageId").find("option:selected").val(),
     				 "page":page
     			 },
     			 dataType:"text",
...
$(function() {
	...
	$.get("FrontRoomBeanServlet?op=queryAllRoom", function(data, status) {
		...
		//遍历数组
		$.each(array, function(index, village) {
			$("#villageId").append(
					"<option value="+village.villageId+">" + village.villageName
							+ "</option>");
		});
	});
});
