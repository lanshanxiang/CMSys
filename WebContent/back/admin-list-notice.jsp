...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 物业服务 <span class="c-gray en">&gt;</span> 小区通知公告信息 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<article class="cl pd-20"> 
		<div class="text-c">
		    ...
		    通知公告内容：<input type="text" class="form-controlSearch input-text " placeholder="输入通知公告内容" data-column="4" id="col4_filter" style="width:100px;">
		     通知公告类别编号：<input type="text" class="form-controlSearch input-text " placeholder="输入通知公告类别编号" data-column="5" id="col5_filter" style="width:100px;">
   		 发布时间：<input type="text" class="form-controlSearch input-text " placeholder="输入发布时间" data-column="6" id="col6_filter" style="width:100px;">
	...
	.prompt(
			...
			function(pass, index) {
				...
				if (pass == "admin") {
					...
					//页面跳转，跳转到servlet
					$.ajax({
						url : "${pageContext.request.contextPath}/NoticeBeanServlet?op=delNotice",//url地址
						type : "post",
						data : {
							"noticeId" : $(obj).parents("tr").find("td").eq(1).text()
						},
						//成功后执行的操作
	 ...
	 $(document).on("click",'.empedit',function()
	 {
	    ...
	    // console.log(arr);
	     ...
	     layer.open({
	 		...
	 		maxmin: true,
	 		shade:0.4,
	 		title: '编辑通知公告表单', //显示的标题
	 		content: 'admin-list-notice-update.jsp', //很多种写法 其中之一直接写目标窗口(要弹出来窗口)
...
<script>
    ...
    employee.requestUrl = {
        queryList:employee.url+"CMSys/NoticeBeanServlet"  //数据是从servlet一侧返回的 json格式
    };
