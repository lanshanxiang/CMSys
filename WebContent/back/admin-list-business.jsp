...
<body>
	<nav class="breadcrumb">
		...
		商家管理 <span class="c-gray en">&gt;</span> 商家列表 <a
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l"><a href="javascript:;" onclick="datadel()"
					...
					class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
						添加商家</a></span> <span class="r">共有数据：<strong><span
						id="datarowcount"></span></strong> 条
				</span>
			</div>
			<ul class="nav nav-tabs" style="margin-top: 30px;">
				<li class="active"><a id="reportY" data-toggle="tab"
					href="#menu1">商家成功入驻</a></li>
				<li><a id="reportYN" data-toggle="tab" href="#menu2">正在审核</a></li>
				<li><a id="reportN" data-toggle="tab" href="#menu3">商家入驻失败</a></li>
			</ul>
			<br>
	...
	<script type="text/javascript">
					...
					.prompt(
							...
							function(pass, index) {
								...
								if (pass == "admin") {
									...
									$
											.ajax({
												url : "${pageContext.request.contextPath}/BusinessBeanServlet?op=deleteBusinessBean",//url地址
												type : "post",
												data : {
													"bId" : $(obj)
															.parents("tr")
		...
		function member_success(obj) {
			$
					.ajax({
						url : "${pageContext.request.contextPath}/BusinessBeanServlet?op=updateState",//url地址
						type : "post",
						data : {
							"bId" : $(obj).parents("tr").find("td").eq(1)
									.text(),
							"bState" : 0
						},
						//成功后执行的操作
					...
					function() {
						...
						layer
								.open({
									...
									shade : 0.4,
									title : '编辑商家信息', //显示的标题
									content : 'business-update.jsp', //很多种写法 其中之一直接写目标窗口(要弹出来窗口)
	...
	<script>
		...
		employee1.requestUrl = {
			queryList : employee1.url
					+ "CMSys/BusinessBeanServlet?op=businessY" //数据是从servlet一侧返回的 json格式
		};
		...
		//        {"width": "10%", "targets": [1]},  //设置第2列宽度
		{
			...
			render : function(data, type, row, meta) {
				if (data == "0") {
					return employee1.property.businessY;
				} else if (data == "1") {
					return employee1.property.businessYN;
				} else if (data == "2") {
					return employee1.property.businessN;
				}
			}
		}, {
			defaultContent : '',
	...
	<script>
		...
		employee2.requestUrl = {
			queryList : employee2.url
					+ "CMSys/BusinessBeanServlet?op=businessYN" //数据是从servlet一侧返回的 json格式
		};
	...
	<script>
		...
		employee3.requestUrl = {
			queryList : employee3.url
					+ "CMSys/BusinessBeanServlet?op=businessN" //数据是从servlet一侧返回的 json格式
		};
