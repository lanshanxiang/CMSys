...
<div class="page-container">
	<article class="cl pd-20"> 
		<div class="text-c">
		       ...
		       通知公告类型名称：<input type="text" class="form-controlSearch input-text " placeholder="输入类型名称" data-column="2" id="col2_filter" style="width:100px;">
		    备注：<input type="text" class="form-controlSearch input-text " placeholder="输入备注" data-column="3" id="col3_filter" style="width:100px;">
   
		   </div>
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="member_add('添加通知公告类型','admin-list-noticetype-add.jsp','','540')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加通知公告类型</a></span> <span class="r">共有数据：<strong><span id="datarowcount"></span></strong> 条</span> </div>
			<div class="mt-20">
	...
	.prompt(
			...
			function(pass, index) {
				...
				if (pass == "admin") {
					...
					//页面跳转，跳转到servlet
					$.ajax({
						url : "${pageContext.request.contextPath}/NoticeTypeBeanServlet?op=delNoticeType",//url地址
						type : "post",
						data : {
							"typeId" : $(obj).parents("tr").find("td").eq(1).text()
						},
						//成功后执行的操作
	 ...
	 $(document).on("click",'.empedit',function()
	 {
	    ...
	    // console.log(arr);
	     ...
	     layer.open({
	 		...
	 		shade:0.4,
	 		title: '编辑物业表单', //显示的标题
	 		content: 'admin-list-noticetype-update.jsp', //很多种写法 其中之一直接写目标窗口(要弹出来窗口)
...
<script>
    ...
    employee.requestUrl = {
        queryList:employee.url+"CMSys/NoticeTypeBeanServlet"   //数据是从servlet一侧返回的 json格式
    };
	...
	function batchIds() {
		...
		if (uuids.length == 0) {
			...
			for (var i = 0; i < uuids.length; i++) {
				typeId.push(uuids[i]['typeId']);
			}
			//这里进行ajax
			$.ajax({
				type : 'POST',
				url : '${pageContext.request.contextPath}/NoticeTypeBeanServlet?op=batchDelete',
				// 传递数组
				data : {
					'typeId' : typeId
				},
