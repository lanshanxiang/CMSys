	...
	<article class="cl pd-20"> 
		<div class="text-c">
				根据内容搜索相应的内容：<input type="checkbox" id="autoSearch"> <br>
				          费用名称：<input type="text" class="form-controlSearch input-text " placeholder="" data-column="2" id="col2_filter" style="width: 100px;"> 
					单价：<input type="text" class="form-controlSearch input-text " placeholder="" data-column="3" id="col3_filter" style="width: 100px;"> 
					 说明：<input type="text" class="form-controlSearch input-text " placeholder="" data-column="4" id="col4_filter" style="width: 100px;">
					 费用类别：<input type="text" class="form-controlSearch input-text " placeholder="" data-column="5" id="col5_filter" style="width: 100px;">
                                                  计价单价：<input type="text" class="form-controlSearch input-text " placeholder="" data-column="6" id="col6_filter" style="width: 100px;">
	...
	.prompt(
			...
			function(pass, index) {
				...
				if (pass == "admin") {
					...
					//页面跳转，跳转到servlet
					$.ajax({
						url : "${pageContext.request.contextPath}/CostServlet?op=del",//url地址
						type : "post",
						data : {
...
<script>
    ...
    employee.requestUrl = {
        queryList:employee.url+"CMSys/CostServlet"  //数据是从servlet一侧返回的 json格式
    };
	...
	function batchIds() {
		...
		if (uuids.length == 0) {
			...
			for (var i = 0; i < uuids.length; i++) {
				costId.push(uuids[i]['costId']);
			}
			//这里进行ajax
			$.ajax({
				type : 'POST',
				url : '${pageContext.request.contextPath}/CostServlet?op=batchDelete',
				// 传递数组
				data : {
					'costId' : costId
				},
