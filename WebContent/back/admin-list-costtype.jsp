	...
	<article class="cl pd-20"> 
		<div class="text-c">
				根据内容搜索相应的内容：<input type="checkbox" id="autoSearch"> <br>
				          费用类型：<input type="text" class="form-controlSearch input-text " placeholder="" data-column="2" id="col2_filter" style="width: 100px;"> 
				          备注：<input type="text" class="form-controlSearch input-text " placeholder="" data-column="3" id="col3_filter" style="width: 100px;">
			</div>
	...
	function batchIds() {
		...
		if (uuids.length == 0) {
			...
			for (var i = 0; i < uuids.length; i++) {
				ctId.push(uuids[i]['ctId']);
			}
			//这里进行ajax
			$.ajax({
				type : 'POST',
				url : '${pageContext.request.contextPath}/CostTypeServlet?op=batchDelete',
				// 传递数组
				data : {
					'ctId' : ctId
				},
