...
public class UserBeanDaoImpl implements UserBeanDao {
	...
	public boolean updateUser(UserBean ub) {
		// TODO Auto-generated method stub
		return DBUtil.execute("UPDATE tb_user SET userName=?,userSex=?,userAge=?,tenementId=?,register=?,question=?,answer=?,state=? WHERE useruserId=?", 
				ub.getUserName(),ub.getUserSex(),ub.getUserAge(),ub.getTenementName(),ub.getRegister(),ub.getQuestion(),ub.getAnswer(),ub.getState(),ub.getUserId())>0;
	}
